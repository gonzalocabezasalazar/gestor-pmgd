...
def cargar_datos_mediciones():
    """Carga el histórico de mediciones I-V"""
    sheet = conectar_google_sheets("DB_MEDICIONES")
    try:
        data = sheet.get_all_records()
        if not data: return pd.DataFrame()
        df = pd.DataFrame(data)
        # Asegurar nombres de columnas (Si la hoja es nueva puede que no tenga encabezados correctos)
        # Se asume estructura: Fecha, Planta, Equipo, String_ID, Amperios
        if 'Fecha' in df.columns: df['Fecha'] = pd.to_datetime(df['Fecha'])
        if 'Amperios' in df.columns: df['Amperios'] = pd.to_numeric(df['Amperios'], errors='coerce').fillna(0)
        return df
    except: return pd.DataFrame()
...
def guardar_medicion_masiva(df_mediciones, planta, equipo, fecha):
    ...
    st.cache_data.clear() # Limpiamos caché para ver los datos en el reporte
...
def crear_pdf_gerencial(planta, periodo, kpis, ia_text, engineer_text, fig_rank, fig_pie):
    ...
    pdf.set_font("Arial", "B", 12); pdf.cell(0, 10, clean_text(f"Reporte de Fallas: {planta} | {periodo}"), 0, 1, 'L')
    ...
    pdf.set_font("Arial", "B", 12); pdf.cell(0, 10, clean_text("Diagnostico Automatico (IA)"), 0, 1, 'L'); pdf.set_font("Arial", "", 10); pdf.multi_cell(0, 7, clean_text(ia_text)); pdf.ln(5)
    pdf.set_font("Arial", "B", 12); pdf.set_text_color(200, 0, 0); pdf.cell(0, 10, clean_text("Conclusiones Gerencia"), 0, 1, 'L'); pdf.set_text_color(0, 0, 0); pdf.set_font("Arial", "", 10); pdf.multi_cell(0, 7, clean_text(engineer_text) if engineer_text else "Sin observaciones."); pdf.ln(10)
    try:
...
# 2. PDF MEDICIONES INDIVIDUAL (CERTIFICADO DE CAJA)
def crear_pdf_mediciones(planta, equipo, fecha, df_data, kpis, comentarios_gerencia, fig_box, evidencias):
    pdf = PDF()
    pdf.add_page()
    pdf.set_auto_page_break(auto=True, margin=15)
    pdf.set_font("Arial", "B", 12); pdf.cell(0, 10, clean_text(f"REPORTE DE MEDICION (Curvas I-V)"), 0, 1, 'C'); pdf.ln(5)
    pdf.set_font("Arial", "", 10); pdf.cell(0, 8, clean_text(f"Planta: {planta}"), 0, 1); pdf.cell(0, 8, clean_text(f"Equipo Auditado: {equipo}"), 0, 1); pdf.cell(0, 8, clean_text(f"Fecha: {fecha}"), 0, 1); pdf.ln(5)
    ...
    pdf.set_font("Arial", "B", 12); pdf.set_text_color(0, 50, 150); pdf.cell(0, 10, clean_text("Informe a Gerencia"), 0, 1, 'L'); pdf.set_text_color(0, 0, 0); pdf.set_font("Arial", "", 10); pdf.multi_cell(0, 6, clean_text(comentarios_gerencia) if comentarios_gerencia else "Sin observaciones."); pdf.ln(10)
    try:
    ...
    for index, row in df_data.iterrows():
        ...
        state = "CRITICO" if row['Estado'] == 'CRÍTICO' else "OK"
        if state == "CRITICO": pdf.set_text_color(200, 0, 0)
        else: pdf.set_text_color(0, 100, 0)
        pdf.cell(60, 8, state, 1, 1, 'C'); pdf.set_text_color(0, 0, 0)
    if evidencias:
...
with tab2:
    ...
    with col_stats:
        ...
        if not vals_clean.empty:
            ...
            if evidencias: st.caption(f"{len(evidencias)} fotos adjuntas.")
...
# --- TAB 3: ESTADÍSTICAS E INFORMES (MEJORADO) ---
with tab3:
    st.header("📊 Centro de Informes y Estadísticas")
    
    # SELECTOR DE MODO
    modo_informe = st.radio("Seleccionar Tipo de Informe:", 
                           ["📉 Reporte de Fallas (Fusibles)", "⚡ Reporte de Mediciones (Protocolos)"], 
                           horizontal=True)
    st.divider()

    if "Fallas" in modo_informe:
        # --- LÓGICA ANTIGUA DE FALLAS ---
        ...
        if not df.empty:
            ...
            with col_kpi:
                # KPIS y Gráficos (Código existente optimizado)
            ...
            c1, c2, c3 = st.columns(3); l_cfg = dict(margin=dict(l=10, r=10, t=30, b=10), height=350); fig_rank=None; fig_pie=None
            with c1:
                if not df_f.empty: 
        ...
        # --- NUEVA LÓGICA DE MEDICIONES (CONSOLIDADO) ---
        df_med = st.session_state.df_med_cache
        if not df_med.empty:
            df_m_plant = df_med[df_med['Planta'] == planta_sel]
            
            st.info("📊 Aquí puedes descargar el Protocolo de Pruebas completo con todas las cajas medidas.")
            
            k1, k2 = st.columns(2)
            k1.metric("Total Strings Medidos", len(df_m_plant))
            k2.metric("Total Cajas Auditadas", df_m_plant['Equipo'].nunique() if 'Equipo' in df_m_plant.columns else 0)
            
            st.dataframe(df_m_plant, use_container_width=True)
            
            xls_data = generar_excel_maestro_mediciones(df_m_plant, planta_sel)
            st.download_button("📥 Descargar Protocolo Excel Completo (Todas las Cajas)", xls_data, f"Protocolo_Mediciones_{planta_sel}.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", type="primary")
        else:
            st.warning("No hay mediciones guardadas aún.")
