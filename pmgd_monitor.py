...
def generar_diagnostico_mediciones(df):
    ...
    if c0 or not cb.empty:
        stt, col, det = "ANOMALIA", "error", "Se detectaron anomalías."
...
with t1:
    ...
    if not df_s.empty:
        for idx, r in df_s.tail(5).sort_index(ascending=False).iterrows():
            ...
            if cols[5].button("🗑️", key=f"d{idx}"):
                borrar_registro(idx)
                st.rerun()
...
with t2:
    ...
    if not v_cl.empty:
        ...
        fig = px.bar(
            ...
            y='Amperios',
            color='Estado',
            color_discrete_map={'OK': '#2e86c1', 'CRÍTICO': '#e74c3c'}
        )
        fig.update_layout(
            template="plotly",
            paper_bgcolor='white',
    ...
    if mode == "Fallas":
        ...
        if not df_f.empty:
            ...
            with c_k:
                ...
                k3.metric("Equipo Crítico", kpis['critico'])
            ...
            if st.download_button("📄 PDF Reporte", crear_pdf_gerencial(planta_sel, fecha_texto, kpis, ia, txt, frk, fpi, fpo), "Reporte.pdf"):
                pass
