...
def crear_pdf_mediciones(planta, equipo, fecha, df_data, kpis, comentarios, fig_box, evidencias):
    ...
    pdf.set_font("Arial", "B", 12); pdf.set_text_color(0, 50, 150); pdf.cell(0, 10, clean_text("Informe a Gerencia"), 0, 1)
    pdf.set_text_color(0, 0, 0); pdf.set_font("Arial", "", 10); pdf.multi_cell(0, 6, clean_text(comentarios) if comentarios else "-"); pdf.ln(10)
    try:
        with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as t1: 
...
with t2:
    ...
    if not v_cl.empty:
        ...
        fig = px.bar(df_ed, x='String ID', y='Amperios', color='Estado', color_discrete_map={'OK':'#2e86c1','CRÍTICO':'#e74c3c'}, template="plotly_white")
...
with t3:
    ...
    if mode == "Fallas":
        ...
        if not df_f.empty:
            ...
            frk=px.bar(drk, x='Fallas', y='Equipo_Full', orientation='h', title="Ranking", template="plotly_white"); frk.update_layout(**l_cfg); c1.plotly_chart(frk, use_container_width=True)
            
            fpi=px.pie(df_f, names='Inversor', title="Inversores", color_discrete_sequence=px.colors.qualitative.Prism, template="plotly_white"); fpi.update_layout(**l_cfg); c2.plotly_chart(fpi, use_container_width=True)
            ...
            # --- NUEVA SECCIÓN: BOTONES PDF Y EXCEL ---
            c_pdf, c_xls = st.columns(2)
            with c_pdf:
                if st.download_button("📄 Descargar PDF Gerencial", crear_pdf_gerencial(planta_sel, fecha_texto, kpis, ia, txt, frk, fpi, fpo), "Reporte_Fallas.pdf", type="primary"): pass
            with c_xls:
                excel_data = generar_excel_pro(df_f, planta_sel, fecha_texto, txt)
                if excel_data:
                    st.download_button("📥 Descargar Excel (Datos)", excel_data, f"Datos_Fallas_{planta_sel}.xlsx")
